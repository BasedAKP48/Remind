...
exports.parse = (text, msg, command) => new Promise((res, rej) => {
  ...
  const options = getOptions(text.trim().split(' '));
  ...
  Promise.resolve(parseTime(options.time, now))
    ...
    .then((time) => {
      ...
      let pndMsg = '';
      if (options.mention) {
        if (options.mention.toLowerCase() !== 'none') {
      ...
      // Discord gets embedded
      if (msg.data.connectorType !== 'discord') {
        pndMsg += ` ${options.message}`;
      }
      delete msg.data;
      const pending = {
        time,
        reply: utils.getReply(msg, reminder.cid, pndMsg.trim(), data),
      };
      root.push(pending);
    }).catch(rej);
});
